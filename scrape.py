...
def fetch_kumalog_aomori_sightings_csv(
    ...
    # 2. info_type_id を確実に文字列に変換した上で、対応表を使って一括変換します
    df['sighting_condition'] = df['info_type_id'].astype(str).map(type_mapping)

    # 3. 万が一、1〜4以外の謎の番号や空欄があった場合は、デフォルトで '目撃' にします
    df['sighting_condition'] = df['sighting_condition'].fillna('目撃')
